...
# 接口参数破解
寻找接口非常简单，看我前言里面的文章就知道怎么找了，最重要的就是参数的破解。

![在这里插入图片描述](https://img-blog.csdnimg.cn/20201016094804864.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQyMjc5MDc3,size_16,color_FFFFFF,t_70#pic_center)
|参数名|作用|值|
|--|--|--|
|category|全部(用户文章类型)|profile_all|
|utm_source|用户来源|toutiao|
|visit_user_token|用户token|MS4wLjABAAAAvazHMceCo3MeM9IJbll231AC8GkJDcrd__iZFw2hi4o(直接从用户链接中获取)|
|max_behot_time|翻页参数|0(默认为0，后续为获得结果的max_behot_time值)|
|_signature|加密参数，用来对比访问的哪个接口|_02B4Z6wo00f01P6um9gAAIBD5.B97mLbnez-q59AAGApeUF3pSlwtwSgMM31ERSbUq4nAurGAbdCsEa34Q4Sbqq1YuL5lnFzvinOS2JFHeprAoHuhyeKjBh6n9y3IR1rlsQc93WseDebgDDp54(加密算法算出来)|

从这里能看出，只有`_signature`参数值是经过加密的，其他的都是可以直接获取到的，所以这里只对`_signature`进行破解，其他参数在后面的代码中有获取过程。

### 逆向_signature参数
直接打断点进行调式，发现`_signature`参数值就是在这里产生的。

![在这里插入图片描述](https://img-blog.csdnimg.cn/20201016100000970.png#pic_center)
我们往上调式，看看他在那一步进行的运算。

![在这里插入图片描述](https://img-blog.csdnimg.cn/2020101610030343.png#pic_center)
发现运算的步骤在这，这下就简单多了，直接进入源码，然后把源码拷贝下来，模拟操作获得结果即可。
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201016100713787.png#pic_center)
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201016100419306.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQyMjc5MDc3,size_16,color_FFFFFF,t_70#pic_center)
`JS源码：`
```javascript
var glb;
...
后续操作只需要实现即可，可以加群(`855262907`)后咨询我，也可以自己摸索，无私奉献文章，加个群都不能吗？

后续操作就简单了，不麻烦了，直接群里咨询我即可。

最重要的参数加密都给你们解决了，能自己解决的就自己解决，不能的就加群。

### 爬取结果
给你们看看爬取的结果。

![在这里插入图片描述](https://img-blog.csdnimg.cn/20201016102355384.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQyMjc5MDc3,size_16,color_FFFFFF,t_70#pic_center)
